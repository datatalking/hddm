...
def TEST_wiener_like_rlddm_nn(str model, 
                      ...
                      # v, a, z, t, alpha, pos_alpha
                    #   double alpha, double pos_alpha, double v, 
                    #   double sv, double a, double z, double sz, double t,
                    #   double st, 
    ...
    cdef np.ndarray[long, ndim=1] unique = np.unique(split_by)
    cdef Py_ssize_t n_params = model_config[model]['n_params'] #4 #params.shape[0]
